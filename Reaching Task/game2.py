...
mask_mode= True if sys.argv[1] == "1" else False
experiment_design = sys.argv[4] # base, leftvright
...
while running:
    ...
    if experiment_design == "base":
        if attempts == 1:
            ...
            running = False
    elif experiment_design == "leftvright":
        if attempts == 1:
            perturbation_mode = False
        elif attempts == 40:
            perturbation_mode = True
    ...
    if check_target_reached():
        ...
        error_angle = error_angle = math.atan2(circle_pos[1] - START_POSITION[1], circle_pos[0] - START_POSITION[0]) - math.atan2(new_target[1] - START_POSITION[1], new_target[0] - START_POSITION[0])
...
# Generate playing field
    ...
    if experiment_design == "leftvright":
        if attempts < 160 or attempts >= 240:
            # show use your right hand!
            font = pygame.font.Font(None, 36)
            hand_text = font.render('USE YOUR RIGHT HAND!', True, black)
            screen.blit(hand_text, (WIDTH//2 - 100, 10))
        elif attempts >= 160 or attempts < 240:
            font = pygame.font.Font(None, 36)
            hand_text = font.render('USE YOUR LEFT HAND!', True, black)
            # text_rect = text.get_rect(center=(START_POSITION[0], 1))
            screen.blit(hand_text, (WIDTH//2 - 100, 10))
    
    if sys.argv[3] == "debug" and len(error_angles) > 0:
...
np.savetxt('motor_variabilities_' + date_str + '.csv', motor_variabilities, delimiter=",")

# np.save('move_faster' + date_str + '.npy', move_faster_events)
# np.save('motor_variabilities' + date_str + '.npy', motor_variabilities)
